...
ZFramework 资源模块是基于 **YooAsset** 构建的上层资源管理框架，提供了**完全自动化的资源引用管理**和**商业级的资源生命周期控制**。相比直接使用 YooAsset，ZFramework 的资源模块具有以下核心优势：
...
ZFramework 资源模块特别优化了对各种小游戏平台的支持：
...
### ZFrameworkSetting 配置

在菜单窗口 `ZFrameworkSetting` 可以设置资源模块的热更新配置：
...
**AssetsReference** 是 ZFramework 资源模块的核心特性，实现了**完全自动化的资源引用管理**。
...
ZFramework 在 YooAsset 基础上构建了资源对象池系统，提供：
...
ZFramework 资源模块提供了 **SetSprite** 扩展接口，实现了**完全自动化的图片设置和引用管理**。
