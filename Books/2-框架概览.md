...
ZFramework 是基于开源框架 **[TEngine](https://github.com/Alex-Rachel/TEngine)** 扩展的个人架构；TEngine 本身建立在 **GameFramework** 的设计基础上。ZFramework 在此基础上继续做精简、商业化适配与工程扩展，并采用**面向接口编程**的设计理念，提供高内聚、低耦合的模块化架构。
...
```
Assets/GameScripts
├── GameEntry.cs        // Assembly-CSharp 启动入口
├── Procedure           // Assembly-CSharp 启动流程
└── HotFix              // 历史兼容目录名
    ├── GameProto       // 游戏配置协议普通程序集
    └── GameLogic       // 游戏业务逻辑普通程序集
        └── GameApp.cs  // 游戏主入口（partial class，可拓展）
```

### 设计说明

- **Assembly-CSharp**：包含 GameEntry 和流程管理
- **GameProto**：游戏配置协议程序集，存放与服务器交互的协议以及配置表逻辑
- **GameLogic**：游戏业务逻辑程序集，存放主要玩法逻辑和 UI

> **提示**：`HotFix` 仅为历史目录名。GameProto 与 GameLogic 当前都是随 Player 构建的普通 Unity 程序集；增删程序集时只需维护 asmdef 引用关系。DLC/Mod 内容通过独立 YooAsset Package 扩展，不动态加载托管 DLL。

---

## 🎯 MVE 架构支持

ZFramework 框架支持 **MVE（Model - View - Event）事件驱动架构**：
...
- [介绍文档](./0-介绍.md) - 了解 ZFramework 核心特性
