...
一个游戏 70% 都是 UI，剩下 30% 才是 GamePlay。ZFramework UI 模块是一套**轻量级、高可拓展性**的商业化 UI 框架，配合事件模块实现 **MVE（Model - View - Event）事件驱动架构**，能够提高至少一倍的开发效率。
...
有自定义需求可以在 `ZFrameworkSetting` 下进行自定义：
...
**UIWidget** 是 ZFramework UI 框架中的核心组件设计，采用**组合优于继承**的设计理念，通过 Widget 的组合来构建复杂的 UI 界面。
...
ZFramework UI 框架支持多层次的 Widget 嵌套结构：
