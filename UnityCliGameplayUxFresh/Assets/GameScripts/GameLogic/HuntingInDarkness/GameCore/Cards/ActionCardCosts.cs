...
namespace HuntingInDarkness.GameCore.Cards
{
    ...
    public sealed class ActionCardResourcePool
    {
        ...
        public int GetCombatInspiration(int ownerId) =>
            GetMind(ownerId)?.InspirationCount ?? 0;
        ...
        public bool TrySpendCombatInspiration(int ownerId, int amount)
        {
            ...
            List<CombatInspirationToken> tokens = GetSpendableTokens(ownerId, InspirationRequirement.Any);
        ...
        public int AddCombatInspiration(int ownerId, int amount)
        {
            ...
            if (amount < 0)
            {
                List<CombatInspirationToken> spendable = mind.GetSpendable(InspirationRequirement.Any);
                int spendCount = Math.Min(-amount, spendable.Count);
                var tokenIds = new List<int>(spendCount);
                for (int index = 0; index < spendCount; index++)
                    tokenIds.Add(spendable[index].Id);
                mind.TrySpend(tokenIds);
                return mind.InspirationCount;
            }
        ...
        public InspirationGain TryAddMindBurden(int ownerId, string displayName, bool isReplaceable = false, int replaceTokenId = -1)
        {
            CombatInspirationMind mind = GetMind(ownerId);
            return mind == null
                ? new InspirationGain(InspirationGainResult.Rejected, default)
                : mind.TryAddBurden(displayName, isReplaceable, replaceTokenId);
        }

        public List<CombatInspirationToken> GetReplaceableTokens(int ownerId) =>
            GetMind(ownerId)?.GetReplaceableTokens() ?? new List<CombatInspirationToken>();
