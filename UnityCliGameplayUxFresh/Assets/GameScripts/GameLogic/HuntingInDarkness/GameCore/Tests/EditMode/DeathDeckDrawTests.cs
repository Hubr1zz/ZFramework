...
namespace HuntingInDarkness.GameCore.Tests.EditMode
{
    public sealed class DeathDeckDrawTests
    {
        ...
        [Test]
        public void SurvivalEventCard_SurvivesRevealsEventAndAddsDeathCard()
        {
            var deck = new DeathDeck(new[] { new DeathDeckCard(DeathCardType.SurvivalEvent, "lucky-survivor") });

            DeathDrawResult result = deck.Draw(new FirstRandom());

            Assert.That(result.Survived, Is.True);
            Assert.That(result.Card, Is.EqualTo(DeathCardType.SurvivalEvent));
            Assert.That(result.SurvivalEventId, Is.EqualTo("lucky-survivor"));
            Assert.That(deck.SurvivalEventCardCount, Is.EqualTo(1));
