...
namespace HuntingInDarkness.GameCore.Tests
{
    public sealed class BossTargetRulesTests
    {
        ...
        [Test]
        public void DeviatingMoveAndTarget_StacksFateOnOriginalTarget()
        {
            BossIntentDeviationResult result = BossIntentDeviationRules.Evaluate(true, 2, 5, new[] { 2, 5, 8 });

            Assert.That(result.IsMoveDeviation, Is.True);
            Assert.That(result.IsTargetDeviation, Is.True);
            Assert.That(result.FateByHunterId[2], Is.EqualTo(2));
            Assert.That(result.FateByHunterId[5], Is.EqualTo(1));
            Assert.That(result.FateByHunterId[8], Is.EqualTo(1));
        }
