...
namespace HuntingInDarkness.GameCore.Combat
{
    ...
    public sealed class BossIntentDeviationResult
    {
        public bool IsMoveDeviation { get; }
        public bool IsTargetDeviation { get; }
        public IReadOnlyDictionary<int, int> FateByHunterId { get; }

        public BossIntentDeviationResult(bool isMoveDeviation, bool isTargetDeviation, IReadOnlyDictionary<int, int> fateByHunterId)
        {
    ...
    public static class BossIntentDeviationRules
    {
        public static BossIntentDeviationResult Evaluate(bool isMoveDeviation, int expectedTargetId, int selectedTargetId, IReadOnlyList<int> aliveHunterIds)
        {
            bool isTargetDeviation = expectedTargetId >= 0 && selectedTargetId >= 0 && expectedTargetId != selectedTargetId;
            var fate = new Dictionary<int, int>();
            if (isMoveDeviation && aliveHunterIds != null)
                foreach (int hunterId in aliveHunterIds)
                    if (hunterId >= 0)
                        AddFate(fate, hunterId);
            if (isTargetDeviation)
                AddFate(fate, expectedTargetId);
            return new BossIntentDeviationResult(isMoveDeviation, isTargetDeviation, fate);
        }

        private static void AddFate(IDictionary<int, int> fate, int hunterId)
        {
            fate.TryGetValue(hunterId, out int amount);
            fate[hunterId] = amount + 1;
        }
    }
}
