...
namespace HuntingInDarkness.ViewLayer.Combat
{
    public sealed class PlayableInspirationCardView : CardView3D
    {
        ...
        public override string DisplayName => token.IsBurden ? token.DisplayName : CombatInspirationPresentation.GetName(token.Color);

        public static PlayableInspirationCardView Create(CombatInspirationToken token, Transform parent)
        {
            var gameObject = new GameObject(token.IsBurden ? $"MindBurden_{token.Id}" : $"Inspiration_{token.Id}_{token.Color}");
        ...
        protected override void ApplyVisuals()
        {
            ...
            Color color = token.IsBurden ? new Color(0.22f, 0.18f, 0.25f) : GetColor(token.Color);
            ...
            symbolText.text = token.IsBurden ? "锁" : GetSymbol(token.Color);
            symbolText.color = Color.white;
            nameText.text = token.IsBurden ? $"{token.DisplayName}\n占用容量" : CombatInspirationPresentation.GetName(token.Color);
