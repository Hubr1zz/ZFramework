...
namespace HuntingInDarkness.ViewLayer.Tabletop
{
    ...
    public sealed class TabletopHuntDeparturePanel3D : MonoBehaviour
    {
        ...
        private void BuildDestinationCards()
        {
            ...
            if (hasSelection)
            {
                PlayableHuntDestination selectedDestination = destinationAvailability[selectedDestinationIndex].Destination;
                TabletopEventChoiceCard3D selectedCard = TabletopEventChoiceCard3D.Create(transform, new Vector3(-0.8f, 0f, 0f));
                selectedCard.Present(selectedDestination.DisplayName, $"{selectedDestination.Description}\n常见收获 · {selectedDestination.ResourceHint}\n风险 · {selectedDestination.DangerHint}", true, "已选择 · 点击重新展开", () =>
                {
                    selectedDestinationIndex = -1;
                    BuildDestinationCards();
                });
                TabletopEventChoiceCard3D departCard = TabletopEventChoiceCard3D.Create(transform, new Vector3(0.8f, 0f, 0f));
                departCard.Present("出发", "以当前常驻小队进入该地区", true, string.IsNullOrWhiteSpace(destinationStatus) ? "点击启程" : destinationStatus, ConfirmDestination);
                return;
            }
            ...
            for (int index = 0; index < destinationAvailability.Count; index++)
            {
                ...
                cards.Add(new TabletopEventChoicePresentation(destination.DisplayName, body, true, "点击选择", () => SelectDestination(destinationIndex)));
            }

            int count = cards.Count;
            if (count == 0)
            {
                TabletopEventChoiceCard3D unavailableCard = TabletopEventChoiceCard3D.Create(transform, Vector3.zero);
                unavailableCard.Present("暂无可前往地区", "尚未解锁任何狩猎地区。", false, "解锁地区后会自动加入此处", null);
                return;
            }
            for (int index = 0; index < count; index++)
            {
                TabletopEventChoicePresentation presentation = cards[index];
                float x = (index - (count - 1) * 0.5f) * 1.55f;
                TabletopEventChoiceCard3D card = TabletopEventChoiceCard3D.Create(transform, new Vector3(x, 0f, 0f));
