...
namespace UI
{
    ...
    public sealed class RecruitmentPanel3D : WorldSpaceViewPanel
    {
        ...
        private string FormatCost()
        {
            ...
            if (cost == 0 && populationCost == 0) return aliveCount <= 0 ? "无人守火时免费援助" : "无需额外成本";
            string resourceLabel = string.Empty;
            if (cost > 0)
                resourceLabel = catalog?.RecruitmentCostItem != null ? $"{catalog.RecruitmentCostItem.itemName} ×{cost}" : "物资未配置";
