...
namespace HuntingInDarkness.Hunt
{
    ...
    public class HuntMapVisualizer : MonoBehaviour, IHuntTileInteractionPresenter
    {
        ...
        public Transform TabletopInteractionAnchor
        {
            get
            {
                if (interactionAnchorCoordinate.HasValue && _tileObjects.TryGetValue(interactionAnchorCoordinate.Value, out GameObject tileObject) && tileObject != null)
                    return tileObject.transform;
                return squadPawn != null ? squadPawn.transform : transform;
            }
        }
        ...
        public void HandleTileClicked(Vector2Int coordinate)
        {
            ...
            if (tile.State == TileState.Interactable)
            {
                ...
                ConfirmSelectedTile(coordinate);
                return;
            }
        ...
        {
            ClearVisuals();
            if (GetComponent<Cards3D.WorldSpaceTypographyGuard>() == null)
                gameObject.AddComponent<Cards3D.WorldSpaceTypographyGuard>();
            _huntMgr = huntMgr;
            ...
            GetComponent<Cards3D.WorldSpaceTypographyGuard>()?.ApplyNow();
        ...
        private void SelectTile(Vector2Int coordinate)
        {
            Vector2Int? previous = selectedTileCoordinate;
            selectedTileCoordinate = coordinate;
            if (previous.HasValue && _huntMgr.Map.TryGetValue(previous.Value, out HexTileInstance previousTile)) ApplyTileColor(previous.Value, previousTile.State, previousTile);
            if (_huntMgr.Map.TryGetValue(coordinate, out HexTileInstance selectedTile)) ApplyTileColor(coordinate, selectedTile.State, selectedTile);
        }

        private void ConfirmSelectedTile(Vector2Int coordinate)
        {
            ...
            {
                Debug.LogWarning($"[HuntMapVisualizer] 地块 {coordinate} 确认探索时快照已失效。");
                return;
            }
            tileRequestInFlight = true;
            SubmitTileAsync(snapshot).Forget();
        ...
        private async UniTaskVoid SubmitTileAsync(HuntExplorationSnapshot snapshot)
        {
            ...
            try
            {
                ...
                if (!result.Succeeded)
                    failureReason = string.IsNullOrWhiteSpace(result.Reason) ? "探索未能完成，请重试。" : result.Reason;
            }
            catch (System.Exception exception)
            {
                Debug.LogException(exception);
                failureReason = "探索未能完成，请重试。";
            }
            finally
            {
            ...
            if (generation != presentationGeneration || _huntMgr == null) return;
            if (string.IsNullOrWhiteSpace(failureReason))
            {
                selectedTileCoordinate = null;
                return;
            }
            Debug.LogWarning($"[HuntMapVisualizer] {failureReason}");
        }
        ...
        private void ApplyTileColor(Vector2Int coord, TileState state, HexTileInstance tile)
        {
            ...
            if (state == TileState.Locked)
                ...
                c = selectedTileCoordinate == coord ? new Color(0.55f, 0.78f, 1f, 1f) : InteractableColor;
            else  // Revealed
        ...
        {
            if (newState != TileState.Interactable && selectedTileCoordinate == coord) selectedTileCoordinate = null;
        ...
        private void ClearVisuals()
        {
            ...
            tileRequestInFlight = false;
            selectedTileCoordinate = null;
            interactionAnchorCoordinate = null;
            mapIntroCamera?.Skip();
