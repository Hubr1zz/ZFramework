...
namespace UI
{
    ...
    public class WorkshopCraftPanel : WorldSpaceViewPanel
    {
        ...
        public void Open(WorkshopCard3D workshop, Vector3 worldPos)
        {
            ...
            CardDimensions recipeDimensions = CardPrefabRegistry.GetDimensions(typeof(WorkshopRecipeCard3D), new CardDimensions(WorkshopRecipeCard3D.DefaultWidth, WorkshopRecipeCard3D.DefaultHeight, CardView3D.CD));
            float recipeWidth = recipeDimensions.Width;
            float recipeHeight = recipeDimensions.Height;
            float rowStep = recipeWidth + RecipeGap;
            ...
            float w = Mathf.Max(recipeWidth + SidePad, n * rowStep + SidePad);
            float h = recipeHeight + 0.9f;
            SetSize(w, h);
