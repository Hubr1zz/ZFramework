...
namespace UI
{
    ...
    public class BossCardTable
    {
        ...
        {
            List<BossHitLocationCard> shuffledViews = _hitLocationCardViews.FindAll(view => view?.State?.IsPersistentActive != true);
            var positions = new List<Vector3>(shuffledViews.Count);
            foreach (BossHitLocationCard view in shuffledViews) positions.Add(view.BaseLocalPos);
            ...
            for (int i = 0; i < shuffledViews.Count; i++)
                shuffledViews[i].MoveTo(positions[i]);
        }
