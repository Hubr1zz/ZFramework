...
namespace UI
{
    ...
    public class SettlementCameraController : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            controlHintOverlay = controlledCamera.GetComponent<TabletopControlHintOverlay>() ?? controlledCamera.gameObject.AddComponent<TabletopControlHintOverlay>();
        ...
        private void HandleKeyboardPan()
        {
            ...
            focusPoint += movement * (keyboardPanSpeed * GlobalGameSettings.CameraPanSpeed * Time.deltaTime);
            ClampFocusPoint();
            UpdateTargetPose();
        }
        ...
        private void HandleMousePan()
        {
            ...
            if (isMousePanning && Input.GetMouseButton(mousePanButton))
            {
                ...
                Vector3 forward = Vector3.ProjectOnPlane(_targetRot * Vector3.forward, Vector3.up).normalized;
                Vector3 right = Vector3.ProjectOnPlane(_targetRot * Vector3.right, Vector3.up).normalized;
                float worldUnitsPerPixel = 2f * distance * Mathf.Tan(controlledCamera.fieldOfView * 0.5f * Mathf.Deg2Rad) / Mathf.Max(1, Screen.height);
