...
namespace HuntingInDarkness.Adapter.PlayModeTests
{
    public sealed class HunterTabletopInteractionPlayModeTests
    {
        ...
        [UnityTest]
        public IEnumerator SlottedCard_RaycastHitsCardInsteadOfSlotTrigger()
        {
            var cameraObject = new GameObject("SlottedCardRaycastCamera");
            createdObjects.Add(cameraObject);
            Camera camera = cameraObject.AddComponent<Camera>();
            cameraObject.transform.position = new Vector3(0f, 5f, -4f);
            cameraObject.transform.rotation = Quaternion.LookRotation(Vector3.zero - cameraObject.transform.position, Vector3.up);

            var root = new GameObject("SlottedCardRaycastRoot");
            createdObjects.Add(root);
            CardSlot slot = CardSlot.Create(root.transform, Vector3.zero, CardView3D.CW, CardView3D.CH, false, CardCategory.Resource);
            var cardObject = new GameObject("SlottedRaycastCard");
            cardObject.transform.SetParent(root.transform, false);
            TestSlotCard card = cardObject.AddComponent<TestSlotCard>();
            card.Initialize(Vector3.zero);
            slot.PlaceCard(card, root.transform);

            yield return null;
            Physics.SyncTransforms();
            var ray = new Ray(cameraObject.transform.position, (card.transform.position - cameraObject.transform.position).normalized);

            Assert.That(slot.gameObject.layer, Is.EqualTo(LayerMask.NameToLayer("Ignore Raycast")));
            Assert.That(Physics.Raycast(ray, out RaycastHit hit), Is.True);
            Assert.That(hit.collider.GetComponentInParent<CardView3D>(), Is.SameAs(card), hit.collider.name);
        }
