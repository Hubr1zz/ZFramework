...
namespace HuntingInDarkness.Adapter.Tests
{
    public sealed class PlayableCombatInspirationTests
    {
        ...
        [Test]
        public void Mind_LockedBurdenOccupiesCapacityButCannotBeSpentOrReplaced()
        {
            var mind = new CombatInspirationMind(2);
            InspirationGain burden = mind.TryAddBurden("恐惧");
            InspirationGain red = mind.TryAdd(CombatInspirationColor.Red);

            Assert.That(burden.Result, Is.EqualTo(InspirationGainResult.Added));
            Assert.That(mind.InspirationCount, Is.EqualTo(1));
            Assert.That(mind.GetSpendable(InspirationRequirement.Any), Has.Count.EqualTo(1));
            Assert.That(mind.TrySpend(new[] { burden.Token.Id }), Is.False);
            Assert.That(mind.TryAdd(CombatInspirationColor.Blue, burden.Token.Id).Result, Is.EqualTo(InspirationGainResult.RequiresReplacement));
            Assert.That(mind.TryAdd(CombatInspirationColor.Blue, red.Token.Id).Result, Is.EqualTo(InspirationGainResult.Replaced));
        }
        ...
        [Test]
        public void ResourcePool_LegacySpendSkipsLockedBurden()
        {
            var pool = new ActionCardResourcePool();
            pool.Register(1);
            pool.TryAddMindBurden(1, "恐惧");
            pool.TryAddCombatInspiration(1, CombatInspirationColor.Blue);

            Assert.That(pool.TrySpendCombatInspiration(1, 1), Is.True);
            Assert.That(pool.GetCombatInspiration(1), Is.Zero);
            Assert.That(pool.GetTokens(1), Has.Exactly(1).Matches<CombatInspirationToken>(token => token.IsBurden));
        }
