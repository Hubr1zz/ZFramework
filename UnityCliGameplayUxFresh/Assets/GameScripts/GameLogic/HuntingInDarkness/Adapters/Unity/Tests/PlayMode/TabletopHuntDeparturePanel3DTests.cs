...
namespace HuntingInDarkness.Adapter.PlayModeTests
{
    public sealed class TabletopHuntDeparturePanel3DTests
    {
        ...
        [Test]
        public void PresentDestinations_HidesLockedRoutesAndRequiresExplicitSelectionBeforeDeparture()
        {
            ...
            try
            {
                ...
                Assert.That(TryFindDestinationCard(panel, "锁定路线"), Is.Null);
                ...
                FindDestinationCard(panel, "出发").Clicked?.Invoke();
        ...
        [Test]
        public void PresentDestinations_AllLockedShowsSingleUnavailablePlaceholder()
        {
            ...
            try
            {
                ...
                Assert.That(TryFindDestinationCard(panel, "锁定甲"), Is.Null);
                Assert.That(TryFindDestinationCard(panel, "锁定乙"), Is.Null);
                TabletopEventChoiceCard3D unavailableCard = FindDestinationCard(panel, "暂无可前往地区");
                Assert.That(unavailableCard.IsInteractable, Is.False);
                unavailableCard.Clicked?.Invoke();
        ...
        [Test]
        public void DestinationView_DestroyAfterPanelWasAlreadyDestroyed_DoesNotAccessMissingReference()
        {
            var viewObject = new GameObject("DestroyedDepartureView");
            var panelRoot = new GameObject("DestroyedDeparturePanelRoot");
            PlayableHuntDestinationView view = viewObject.AddComponent<PlayableHuntDestinationView>();
            TabletopHuntDeparturePanel3D panel = TabletopHuntDeparturePanel3D.Create(panelRoot.transform);
            typeof(PlayableHuntDestinationView).GetField("panel", BindingFlags.Instance | BindingFlags.NonPublic)?.SetValue(view, panel);

            Object.DestroyImmediate(panel.gameObject);
            Assert.DoesNotThrow(() => Object.DestroyImmediate(viewObject));
        ...
        {
            TabletopEventChoiceCard3D card = TryFindDestinationCard(panel, title);
            if (card != null) return card;
            Assert.Fail($"找不到目的地卡：{title}");
            return null;
        }

        private static TabletopEventChoiceCard3D TryFindDestinationCard(TabletopHuntDeparturePanel3D panel, string title)
        {
            foreach (TabletopEventChoiceCard3D card in panel.GetComponentsInChildren<TabletopEventChoiceCard3D>(true))
                if (card.gameObject.activeSelf && card.DisplayName == title)
                    return card;
            return null;
        }
