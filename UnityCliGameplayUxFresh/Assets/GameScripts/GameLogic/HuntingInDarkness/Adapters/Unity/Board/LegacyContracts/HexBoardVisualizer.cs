...
namespace GameplayBase.Board
{
    ...
    public class HexBoardVisualizer
    {
        ...
        public void HighlightIntent(List<Vector2Int> selectableTiles, List<Vector2Int> compliantTiles)
        {
            ClearHighlights();
            if (selectableTiles != null)
                foreach (Vector2Int tile in selectableTiles)
                    SetHighlight(tile, new Color(0.9f, 0.42f, 0.12f, 1f));
            if (compliantTiles != null)
                foreach (Vector2Int tile in compliantTiles)
        ...
        private void SetHighlight(Vector2Int tile, Color color)
        {
            if (!_tileRenderers.TryGetValue(tile, out Renderer renderer)) return;
