...
namespace HuntingInDarkness.Combat
{
    ...
    public sealed class PlayableDirectedBossAttackEffect : BossActionCardEffect, IPlayableQueuedBossActionEffect, IPlayableCancellableBossActionEffect
    {
        ...
        public PlayableDirectedBossAttackEffect(string actionName, int woundCount, int accuracy, int attackCount, BossTargetPolicy targetPolicy, int movementDistance = 0, int attackRange = int.MaxValue, string noTargetOrRangeFailure = "", string mindBurdenName = "", IRandomSource random = null)
        {
            ...
            this.noTargetOrRangeFailure = string.IsNullOrWhiteSpace(noTargetOrRangeFailure) ? "怪物没有可攻击的目标，本次攻击落空。" : noTargetOrRangeFailure;
        ...
        {
            return new DirectedBossAttackAction(context, actionName, woundCount, accuracy, attackCount, targetPolicy, movementDistance, attackRange, noTargetOrRangeFailure, mindBurdenName, random, eventOutbox, boss, combat, resolveTarget);
        }
