...
namespace HuntingInDarkness.Combat
{
    ...
    public static class PlayableHunterInjuryAdapter
    {
        ...
        public static void Sync(HunterInstance hunter, CharacterCombatStats combatStats)
        {
            ...
            hunter.SurvivalCardEventIds = new List<string>(state.DeathDeck.SurvivalEventIds);
        ...
        {
            hunter.SurvivalCardEventIds ??= new List<string>();
            var survivalEventIds = new List<string>();
            foreach (string eventId in hunter.SurvivalCardEventIds)
                if (!string.IsNullOrWhiteSpace(eventId) && !survivalEventIds.Contains(eventId))
                    survivalEventIds.Add(eventId);
            int persistedSurvivalEventCount = survivalEventIds.Count;
            IReadOnlyList<string> traitCards = PlayableSurvivalEventRuntime.Catalog?.GetSurvivalCardIds(hunter.Traits);
            if (traitCards != null)
                foreach (string eventId in traitCards)
                    if (!string.IsNullOrWhiteSpace(eventId) && !survivalEventIds.Contains(eventId))
                        survivalEventIds.Add(eventId);

            int survivalCount = Math.Max(0, hunter.SurvivalCards - persistedSurvivalEventCount);
            int deathCount = Math.Max(0, hunter.DeathCards);
            if (survivalCount + survivalEventIds.Count + deathCount == 0) survivalCount = 1;

            var cards = new List<DeathDeckCard>(survivalCount + survivalEventIds.Count + deathCount);
            for (int i = 0; i < survivalCount; i++) cards.Add(new DeathDeckCard(DeathCardType.Survive));
            foreach (string eventId in survivalEventIds) cards.Add(new DeathDeckCard(DeathCardType.SurvivalEvent, eventId));
            for (int i = 0; i < deathCount; i++) cards.Add(new DeathDeckCard(DeathCardType.Death));
