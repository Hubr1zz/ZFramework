...
namespace GameplayBase.CombatSystem
{
    ...
    public interface IBossIntentInputProvider
    {
        UniTask<Vector2Int?> RequestBossDestination(string prompt, List<Vector2Int> validTiles, List<Vector2Int> compliantTiles, CancellationToken cancellationToken = default);
        UniTask<int> RequestBossTarget(string prompt, List<int> validTargetIds, List<int> compliantTargetIds, CancellationToken cancellationToken = default);
    }
}
