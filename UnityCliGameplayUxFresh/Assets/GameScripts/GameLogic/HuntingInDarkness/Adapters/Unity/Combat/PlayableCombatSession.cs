...
namespace HuntingInDarkness.Combat
{
    ...
    /// </summary>
    public sealed class PlayableCombatSession : IGameContext, ICombatProvider, ICombatActionCommands, IPlayableActionCardCommandSink, ICombatRuntimeDataProvider, IPlayableBossIntentCommands, IDisposable
    {
        ...
        public bool TryMoveBoss(Vector2Int destination)
        {
            if (disposed || bossData == null || boardQuery == null || boardCommand == null || !boardQuery.IsValidTile(destination)) return false;
            int? occupant = boardQuery.GetEntityAt(destination);
            if (occupant.HasValue && occupant.Value != bossData.Id) return false;
            Vector2Int origin = boardQuery.GetEntityPosition(bossData.Id);
            if (origin == destination) return true;
            boardCommand.MoveEntity(bossData.Id, destination);
            entityVisualizer?.MoveEntity(bossData.Id, destination);
            EventBus.Publish(new EntityMovedEvent { EntityId = bossData.Id, FromTile = origin, ToTile = destination });
            return true;
        }

        public void AddFate(int hunterId, int amount)
        {
            HunterInstance hunter = combatCasualties.GetHunter(hunterId);
            if (hunter == null || amount <= 0) return;
            long next = (long)hunter.Luck + amount;
            hunter.Luck = next > int.MaxValue ? int.MaxValue : (int)next;
        }

        public InspirationGain AddMindBurden(int hunterId, string displayName)
        {
            if (disposed || actionCardCostService == null)
                return new InspirationGain(InspirationGainResult.Rejected, default);
            return actionCardCostService.AddMindBurden(hunterId, displayName);
        }
        ...
        private void InitializeTurnSystem()
        {
            ...
            foreach (CharacterRuntimeData character in characters)
            {
                ...
                actionCardResources.Register(character.Id, character.CombatInspiration, character.CombatMindCapacity);
            ...
            turnStateMachine = new TurnStateMachine(this)
            {
                ...
                GetCharacterActionUnavailableReason = GetCharacterActionUnavailableReason,
                ...
                RequestBossRecovery = AcknowledgeBossRecoveryAsync,
        ...
        private UniTask AcknowledgeBossRecoveryAsync()
        {
        ...
        private void InitializeCardDisplay()
        {
            ...
            PlayableShowdownGuidanceView.Create(scope.Root.transform, OnEndTurn, () => BossRevealedCards);
        }
        ...
        private string GetCharacterActionUnavailableReason(int characterId)
        {
            ...
            {
                TimelineActionStatus.Overtime => "该猎人正在超时：请让队友使用【鼓舞】恢复 1 TP。",
                TimelineActionStatus.Exhausted => "该猎人已超出本轮前摇上限，本轮无法继续行动。",
                TimelineActionStatus.Done => "该猎人本轮已完成行动。",
                _ => "该猎人的累计 TP 当前最高；先让另一名猎人追平，再继续行动。"
            };
        }
    }
}
