...
namespace Core
{
    ...
    public class PlayerTurnState : TurnState
    {
        ...
        private async UniTaskVoid InitializePlayerTurnAsync()
        {
            ...
            try
            {
                ...
                EventBus.Publish(new BossActionStageChangedEvent { Stage = BossActionStage.Windup });
                initialized = true;
            }
            finally
            {
        ...
        public void SelectCharacter(int characterId)
        {
            ...
            {
                string reason = Machine.GetCharacterActionUnavailableReason?.Invoke(characterId);
                EventBus.Publish(new CombatGuidanceNoticeEvent { Message = string.IsNullOrWhiteSpace(reason) ? "该猎人现在不能行动。" : reason });
                return;
            }
    ...
    public class BossTurnState : TurnState
    {
        ...
        public override void Enter()
        {
            ...
            EventBus.Publish(new BossActionStageChangedEvent { Stage = BossActionStage.Execution });
        ...
        private async UniTaskVoid RunBossTurnAsync()
        {
            ...
            try
            {
                ...
                EventBus.Publish(new BossActionStageChangedEvent { Stage = BossActionStage.Recovery });
                if (Machine.RequestBossRecovery != null)
                    await Machine.RequestBossRecovery();
                if (Machine.IsSessionActive != null && !Machine.IsSessionActive()) return;
    ...
    public class TurnStateMachine
    {
        ...
        public System.Func<int, string> GetCharacterActionUnavailableReason;
        ...
        public System.Func<UniTask> RequestBossRecovery;
