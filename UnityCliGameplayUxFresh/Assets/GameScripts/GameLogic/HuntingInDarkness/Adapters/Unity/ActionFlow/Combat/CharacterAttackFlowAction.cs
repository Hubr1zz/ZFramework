...
namespace HuntingInDarkness.ActionFlow.Combat
{
    ...
    internal sealed class CharacterAttackExecution
    {
        ...
        public HitResult? PreviousResult { get; set; }
    ...
    public sealed class DrawHitLocationsAction : CommandAction, ISourceAction, ITargetAction
    {
        ...
        protected override async UniTask<ActionOutcome> ExecuteAsync(ActionExecutionContext context, CancellationToken cancellationToken)
        {
            ...
            List<HitLocationRuntimeState> persistent = all.FindAll(state => state != null && state.IsPersistentActive && !state.IsDestroyed);
            if (persistent.Count > 0 && execution.Input is IPlayerOptionInputProvider optionInput)
            {
                var options = new List<PlayerChoiceOption> { new(0, $"攻击本体（抽取 {drawCount} 个部位）") };
                for (int index = 0; index < persistent.Count; index++)
                    options.Add(new PlayerChoiceOption(index + 1, $"直接击破持续部位：{persistent[index].Data.locationName}"));
                int choice = await optionInput.RequestSelectOption("持续部位仍在场：本次攻击要做什么？", options, cancellationToken: cancellationToken);
                if (choice > 0 && choice <= persistent.Count)
                {
                    execution.Context.RevealedHitLocations = new List<HitLocationRuntimeState> { persistent[choice - 1] };
                    return ActionOutcome.Success("直接攻击持续部位");
                }
            }

            List<HitLocationRuntimeState> available = all.FindAll(state => state != null && !state.IsDestroyed && !state.IsPersistentActive);
    ...
    public sealed class SelectHitLocationAction : CommandAction, ISourceAction, ITargetAction
    {
        ...
        protected override async UniTask<ActionOutcome> ExecuteAsync(ActionExecutionContext context, CancellationToken cancellationToken)
        {
            ...
            string previous = execution.PreviousResult.HasValue ? $"上一部位：{(execution.PreviousResult == HitResult.Success ? "成功" : "失败")}\n" : string.Empty;
            HitLocationRuntimeState selected = await execution.Input.RequestSelectRevealedCard($"{previous}分配{resultName}结果 [{resolution.Index}/{resolution.Count}] — 按结算顺序选择部位", execution.Remaining, cancellationToken);
            ...
            bool previousSucceeded = execution.PreviousResult == HitResult.Success;
            resolution.EffectiveToughness = HitLocationSequenceRules.GetEffectiveToughness(selected.DomainState.Definition, previousSucceeded);
            bool successCardSucceeds = resolution.ResultCard == AttackResultCard.Success && HitLocationSequenceRules.DoesSuccessCardSucceed(selected.DomainState.Definition, execution.Context.TotalAttackPower, previousSucceeded);
            execution.Context.HitResult = successCardSucceeds ? HitResult.Success : HitResult.Failure;
            AttackCheck check = CombatRules.ResolveHitLocationAttack(execution.Context.TotalAttackPower, resolution.EffectiveToughness);
    ...
    public sealed class FinalizeHitLocationResolutionAction : CommandAction, ISourceAction, ITargetAction
    {
        ...
        {
            if (resolution.Selected == null) return ActionOutcome.Failure("受击部位尚未选择");
            if (resolution.Selected.Data.persistent && !resolution.Selected.IsDestroyed)
                resolution.Selected.ActivatePersistent();
            ...
            if (resolution.Selected.Data.previousSuccessToughnessModifier > 0 && execution.PreviousResult == HitResult.Success)
                result += $"\n连续命中使韧性提高到 {resolution.EffectiveToughness}";
            if (resolution.Selected.IsPersistentActive)
                result += "\n持续部位留场：下次攻击可直接选择击破";
            if (execution.Context.GameContext?.Boss is IBossVitalityState vitality)
    ...
    public sealed class HideHitLocationsAction : CommandAction, ISourceAction, ITargetAction
    {
        ...
        protected override UniTask<ActionOutcome> ExecuteAsync(ActionExecutionContext context, CancellationToken cancellationToken)
        {
            ...
            {
                if (state == null || state.IsDestroyed || state.IsPersistentActive) continue;
                state.Hide();
