...
namespace Cards3D
{
    ...
    public abstract class SlotDraggableCardView3D : CardView3D
    {
        ...
        private static float DistanceToSlotArea(CardSlot slot, Vector3 worldPosition)
        {
            Vector3 localPosition = slot.transform.InverseTransformPoint(worldPosition);
            float outsideX = Mathf.Max(Mathf.Abs(localPosition.x) - slot.SlotW * 0.5f, 0f);
            float outsideZ = Mathf.Max(Mathf.Abs(localPosition.z) - slot.SlotH * 0.5f, 0f);
