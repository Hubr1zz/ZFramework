...
namespace Cards3D
{
    ...
    public abstract class WorldSpaceViewPanel : MonoBehaviour
    {
        ...
        [Header("相机取景")]
        [SerializeField] private bool focusCameraOnShow = true;

        protected virtual bool FocusCameraOnShow => focusCameraOnShow;

        // ─── 基础构建（子类在自己的 Build 里调用一次）────────────────────────

        protected void BuildBase()
        {
            cameraFrameLease ??= GetComponent<TabletopCameraFrameLease>() ?? gameObject.AddComponent<TabletopCameraFrameLease>();
        ...
        public void ShowAt(Vector3 worldPos)
        {
            ...
            if (FocusCameraOnShow && TabletopCameraFraming.TryCalculateWorldBounds(transform, out Bounds bounds))
                cameraFrameLease.Acquire(bounds, 50);
        }
