...
namespace Cards3D
{
    ...
    /// </summary>
    public class WorkshopCard3D : SlotDraggableCardView3D
    {
        ...
        {
            WorkshopCard3D card = CardPrefabRegistry.Create<WorkshopCard3D>(parent, $"Workshop_{workshopName}");
        ...
        protected override void BuildTextFields()
        {
            ...
            _descText ??= transform.Find("Desc")?.GetComponent<TextMeshPro>();
            _imageRenderer ??= transform.Find("Image")?.GetComponent<SpriteRenderer>();
            _nameText ??= MakeText("Name", new Vector3(0f, ty, Height * 0.38f), 0.10f, TextAlignmentOptions.Center, new Vector2(Width - 0.06f, 0.20f));
            _descText ??= MakeText("Desc", new Vector3(0f, ty, Height * 0.02f), 0.068f, TextAlignmentOptions.Center, new Vector2(Width - 0.06f, 0.44f));

            if (_imageRenderer == null)
            {
                var imgGo = new GameObject("Image");
                imgGo.transform.SetParent(transform, false);
                imgGo.transform.localPosition = new Vector3(0f, ty + 0.001f, Height * 0.20f);
