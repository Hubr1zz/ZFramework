...
namespace Cards3D
{
    ...
    {
        public const float DefaultWidth = 1.25f;
        public const float DefaultHeight = 1.80f;
        ...
        protected override CardDimensions FallbackDimensions => new(DefaultWidth, DefaultHeight, CD);
        ...
        {
            WorkshopRecipeCard3D card = CardPrefabRegistry.Create<WorkshopRecipeCard3D>(parent, $"Recipe_{recipe?.recipeName}");
            card.recipe = recipe;
        ...
        protected override void BuildTextFields()
        {
            ...
            requirementText ??= transform.Find("Requirements")?.GetComponent<TextMeshPro>();
            hintText ??= transform.Find("Hint")?.GetComponent<TextMeshPro>();
            nameText ??= MakeText("Name", new Vector3(0f, textHeight, Height * 0.38f), 0.13f, TextAlignmentOptions.Center, new Vector2(Width - 0.1f, 0.30f));
            requirementText ??= MakeText("Requirements", new Vector3(0f, textHeight, Height * 0.05f), 0.085f, TextAlignmentOptions.Center, new Vector2(Width - 0.12f, 0.72f));
            hintText ??= MakeText("Hint", new Vector3(0f, textHeight, -Height * 0.40f), 0.075f, TextAlignmentOptions.Center, new Vector2(Width - 0.1f, 0.30f));
        }
