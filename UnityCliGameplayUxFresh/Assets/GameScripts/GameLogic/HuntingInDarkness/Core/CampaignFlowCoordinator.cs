...
namespace Core
{
    ...
    internal sealed class CampaignFlowCoordinator : ICampaignPhaseTransitionHost, ICampaignPhaseTransitionRequestHost, ICampaignRestartHost, ICampaignStartupTransactionHost, ICampaignHuntReturnHost, ICampaignHuntDepartureHost, ICampaignShowdownOutcomeHost, ICampaignEncounterHandoffHost, IPlayableHuntRetreatInput
    {
        ...
        internal bool IsSettlementDeveloperCommandAvailable => CampaignStarted && CurrentPhase == GamePhase.Settlement && SettlementManager != null && SettlementActionSession?.IsActive == true && SettlementActionSession.IsRunning == false;
        ...
        {
            if (!IsSettlementDeveloperCommandAvailable) return null;
        ...
        {
            if (!IsSettlementDeveloperCommandAvailable || string.IsNullOrWhiteSpace(resourceName) || amount <= 0) return false;
            SettlementManager.DevAddResource(resourceName, amount);
            settlementPhase?.Refresh();
            return true;
        }

        internal int DevAddBlankHunters(int amount)
        {
            if (!IsSettlementDeveloperCommandAvailable || amount <= 0) return 0;
            int created = 0;
            for (int index = 0; index < amount; index++)
            {
                HunterInstance hunter = SettlementManager.DevAddHunter(null);
                if (hunter == null) break;
                hunter.Name = $"猎人 {hunter.InstanceId}";
                created++;
            }
            if (created > 0) settlementPhase?.Refresh();
            return created;
        }

        internal bool TryResolveWorkshop(string workshopId, out PlayableWorkshopDefinition definition)
        {
            definition = null;
            if (string.IsNullOrWhiteSpace(workshopId) || bindings.WorkshopCatalog?.Workshops == null) return false;
            string normalizedId = workshopId.Trim();
            foreach (PlayableWorkshopDefinition candidate in bindings.WorkshopCatalog.Workshops)
            {
                if (candidate == null || !string.Equals(candidate.WorkshopId, normalizedId, StringComparison.Ordinal)) continue;
        ...
        internal bool DevUnlockWorkshop(string workshopId, out bool changed)
        {
            changed = false;
            if (!IsSettlementDeveloperCommandAvailable || !TryResolveWorkshop(workshopId, out PlayableWorkshopDefinition definition)) return false;
            changed = !SettlementData.IsWorkshopBuilt(definition.WorkshopId);
            if (changed) SettlementData.BuildWorkshop(definition.WorkshopId);
            settlementPhase?.Refresh();
            return true;
        }

        internal bool DevTriggerSettlementEvent(EventData gameEvent)
        {
            if (!IsSettlementDeveloperCommandAvailable || gameEvent == null || !ReferenceEquals(SettlementManager.Timeline.ResolveEvent(gameEvent.ContentId), gameEvent)) return false;
            return settlementPhase.QueueEvents(SettlementRuntime, SettlementActionSession, new[] { new SettlementEventWork(gameEvent) });
        }
