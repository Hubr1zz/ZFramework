...
namespace Core
{
    ...
    public class GameManager : MonoBehaviour
    {
        ...
        [SerializeField, Tooltip("允许在运行时按 Tab 打开 GM 指令面板。与开发阶段跳转解耦，可单独关闭。")]
        private bool gmPanelEnabled = true;
        ...
        private void Start()
        {
            ...
            // GM 面板独立于开发起始阶段，默认可用并可由 Inspector 关闭。
            if (gmPanelEnabled)
                EnsureDevPanel();
            EnsureGlobalSettingsMenu();
        ...
        private void EnsureGlobalSettingsMenu()
        {
            GameObject parent = uiShared != null ? uiShared : gameObject;
            GlobalSettingsMenu menu = parent.GetComponentInChildren<GlobalSettingsMenu>(true);
            if (menu == null)
            {
                GameObject menuObject = new("GlobalSettingsMenu", typeof(RectTransform));
                menuObject.transform.SetParent(parent.transform, false);
                RectTransform rect = menuObject.GetComponent<RectTransform>();
                rect.anchorMin = Vector2.zero;
                rect.anchorMax = Vector2.one;
                rect.offsetMin = Vector2.zero;
                rect.offsetMax = Vector2.zero;
                menu = menuObject.AddComponent<GlobalSettingsMenu>();
            }
            menu.Initialize();
        }
