...
namespace ZFramework.Editor.Inspector
{
    ...
    internal sealed class ResourceModuleDriverInspector : GameFrameworkInspector
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            // Unity 6 不允许对运行时实例或 DontSaveInEditor 临时对象执行
            // SerializedProperty 的持久化绘制，否则会触发对象持久化断言。
            // 编辑模式下的场景/Prefab 对象仍使用完整配置面板。
            bool isNonPersistentTarget = EditorApplication.isPlaying ||
                                         (t.hideFlags & HideFlags.DontSaveInEditor) != 0;
            if (isNonPersistentTarget)
            {
                EditorGUILayout.HelpBox(
                    "当前是运行时或临时对象，资源模块配置不可持久化编辑。请停止运行后编辑场景或 Prefab 中的 ResourceModuleDriver。",
                    MessageType.Info);
                return;
            }
        ...
        private void DrawInspectorHeader()
        {
            ...
            EditorGUILayout.LabelField(new GUIContent("ZFramework资源模块配置", "Resource Module Configuration"),
