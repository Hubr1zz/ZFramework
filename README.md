...
- 🧱 **标准程序集** - 游戏逻辑随 Player 编译，减少单机项目的构建复杂度
...
- ProcedureInitResources → ProcedureInitContentPackages
...
远程下载步骤只在 Host/Web 内容模式使用，单机模式会跳过；`ProcedureInitContentPackages` 为后续 DLC/Mod 初始化保留。
