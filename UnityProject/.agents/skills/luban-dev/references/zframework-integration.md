# ZFramework 框架 Luban 集成指南

## 概述

ZFramework 项目使用 Luban 作为配置表方案，生成格式为 **cs-bin**（C# 代码）+ **bin**（二进制数据），通过 YooAsset 资源系统加载二进制数据，生成代码位于普通 Player 程序集 `GameScripts/HotFix/GameProto/`。
...
UnityProject/Assets/
├── GameScripts/HotFix/GameProto/             # GameProto 普通程序集（生成代码）
...
`ConfigSystem.cs` 桥接 Luban 生成代码与 ZFramework 的 YooAsset 资源系统：
