...
- Git `changed` 查询使用 `core.quotepath=false` 获取 UTF-8 路径，并将仓库根路径严格映射为项目根相对路径；Unity 项目位于更高层 Git 仓库的子目录时也能正确匹配索引，项目外改动会被忽略。中文文件名能够正常进入索引和变更结果，但团队仍可采用英文文件名降低跨工具、终端和外部插件的兼容风险。
