...
每次输出包含 `engine=codebase-query-regex-binding-v4` 与 `schemaVersion=4`，供 Agent 和人工核验查询来源。构建进度输出到 stderr，stdout 始终只包含最终 JSON。索引通过同目录临时文件原子替换，构建中断不会留下半份正式缓存。
...
索引 v4 使用单文件一次提取、文件级增量复用和轻量 C# 类型绑定，但仍不是 Roslyn 语义模型。
