...
function Add-CSharpTypeBindings {
    ...
    Write-CodebaseQueryProgress -Message 'binding symbol tables ready'
    ...
    Write-CodebaseQueryProgress -Message 'binding base types ready'
    ...
    for ($recordIndex = 0; $recordIndex -lt $recordArray.Count; $recordIndex++) {
        ...
        if (($recordIndex + 1) % 25 -eq 0 -or $recordIndex + 1 -eq $recordArray.Count) {
            Write-CodebaseQueryProgress -Message "binding $($recordIndex + 1)/$($recordArray.Count) files"
        }
    }
