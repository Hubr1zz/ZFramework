...
- **类型**: 表现稳定性 / 全局资源配置
- **描述**: Unity 运行验证中，默认 LiberationSans SDF 无法覆盖中文，产生缺字方框并持续刷出 Missing Character 警告。应集中建立覆盖项目常用中文字形的 TMP 字体资产，并在 TMP Settings 配置全局 fallback；各 View 只引用统一字体配置，避免逐标签修补。实体骰子已支持通过序列化字段指定结果字体，但不应成为全局问题的局部替代。
...
### [优先级: 中] Hunt 相机边界应由地图投影动态配置
- **文件**: `Assets/GameScripts/GameLogic/HuntingInDarkness/ViewLayer/Hunt/World/HuntCameraController.cs`
- **类型**: 硬编码 / 可扩展性
- **描述**: 相机平移仍固定夹在 `mapExtent = 8` 的原点对称范围内，没有消费当前地区地图的实际世界边界。现有半径 3、原点居中的地图可正常使用，但未来地区扩大、组合地块改变尺寸或地图中心偏移后，相机会把玩家重新夹离真实内容。应由 Hunt 地图 Adapter 投影边界与视口留白，再在每次地图生成后配置控制器；保持相机 View 不读取权威地图规则。
- **来源**: 2026-08-20 狩猎地图入场运镜实现审查
- **状态**: 待处理
- **维护人**: codex
- **维护时间**: 2026-08-20
- **维护备注**: 当前地图规模不触发；待地区地图尺寸出现差异时与相机输入边界一次性调整，避免局部反复硬编码。

### [优先级: 中] 存档文件缺少崩溃安全的原子替换
- **文件**: `Assets/GameScripts/GameLogic/HuntingInDarkness/Adapters/Unity/Persistence/SaveLoadSystem.cs`
- **类型**: 数据可靠性 / 存档演进
- **描述**: 正常操作和应用退出已通过版本门禁保存最新 SettlementInstance，但最终仍使用 `File.WriteAllText` 直接覆盖正式 JSON；进程崩溃、磁盘写入失败或断电可能留下半份文件。后续完整分析存档演进需求后，应加入临时文件写入、刷新、原子替换、上一份备份和带 schemaVersion 的存档封套，并定义损坏后的回退策略；不要在各调用点分别补救。
- **来源**: 2026-08-20 战役退出持久化审查
- **状态**: 已处理
- **维护人**: leonz
- **维护时间**: 2026-08-20
- **维护备注**: 2026-08-20 已完成：同目录临时文件刷盘后原子替换正式档，上一版保留为备份；带 Magic Header、schemaVersion 与 SHA-256 的封套拒绝截断数据，读取自动回退备份并兼容旧裸 JSON。删除先推进版本门禁，再清理正式、备份与临时文件。
...
- **维护人**: codex
- **维护时间**: 2026-08-20
- **维护备注**: 已为 `ItemData` 增加显式 ContentId，并将资源、装备、事件、狩猎、制作与 3D 卡牌边界统一为 ID；旧显示名存档在内容注册后幂等合并迁移，未知内容保留，未来 schema 不降级，歧义 ID/显示名整组拒绝。显示名仅保留为表现与兼容别名。

### [优先级: 高] 发明状态与跨内容引用迁移到稳定 ContentId
- **文件**: `Adapters/Unity/Data/SettlementData.cs`、`Adapters/Unity/Settlement/Legacy/InventionSystem.cs`、发明/工坊/训练 Action 与事件 `UnlockInvention` 效果
- **类型**: 内容身份 / 存档演进
- **描述**: `UnlockedInventions`、前置/互斥、工坊要求、武器训练与事件效果仍以 `inventionName` 显示文本作为身份。当前年鉴使用资产名生成独立记录 ID，避免继续扩大显示名依赖，但不能解决发明改名、本地化与未来读表引用。引入发明表前应完整分析这些入口，为 `InventionData` 增加显式 ContentId，并提供旧名称幂等迁移；不得只修改解锁列表。
- **来源**: 2026-08-20 发明年鉴投影实现审查
- **状态**: 已完成
- **维护人**: codex
- **维护时间**: 2026-08-20
- **维护备注**: 已一次迁移 `UnlockedInventions`、前置/互斥、配方、工坊、训练、事件、ActionQueue 实体/事务与年鉴记录；旧显示名和资产名会幂等合并到稳定 ID，未知内容保留，未来 schema 不降级，歧义目录在玩法装配前拒绝。
