...
本 skill 只约束当前项目中的 zWorkFlow Workbench 改动。开始修改前先读取目标页面及其共享渲染方法，完成后运行布局审计和 Unity MCP 编译验证。
...
5. 新增或修改任一页面后，至少验证 Workbench 的 `900x600` 最小窗口和一个常用较大尺寸。优先使用数据化布局审计与 Unity MCP，不以截图作为主要验证手段。
