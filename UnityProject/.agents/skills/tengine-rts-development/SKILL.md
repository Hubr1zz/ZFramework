...
- Data must not reference Unity, TEngine runtime, RTS contracts, `GameObject`, `MonoBehaviour`, or engine time APIs.
- There must be exactly one implementation of each gameplay rule. Do not duplicate rules under `TENGINE_RTS` and `UNITY_5_3_OR_NEWER` branches.
