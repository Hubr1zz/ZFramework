#!/usr/bin/env python3
"""Generate concise TEngine architecture docs from the current Unity project."""
...
def collect() -> dict:
    ...
    implementations = []
    for path in (ASSETS / "TEngine/Runtime/Module").rglob("*.cs"):
...
TEngine 当前采用 Unity Player 原生程序集、TEngine 模块系统、YooAsset 资源管理、UniTask 异步和 Luban 配置表。不使用托管代码热更新或代码混淆插件。
...
Assembly-CSharp / Launcher  启动流程与启动 UI
          ↓
TEngine.Runtime             模块、资源、事件、FSM、对象池
          ↓
...
2. 使用 YooAsset Builder 或 TEngine ReleaseTools 构建 `DefaultPackage`。
3. 按目标平台构建 Player。
