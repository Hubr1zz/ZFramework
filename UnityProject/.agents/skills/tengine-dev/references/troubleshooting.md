...
> 解决新问题后记录到 `.agents/skills/tengine-dev/references/problems/problem_YYYY-MM-DD.md`
...
确认依赖方向是 `GameLogic → GameProto → TEngine.Runtime`。自定义 asmdef 不能引用 `Assembly-CSharp` 中的类型；需要共享的接口应下沉到 `TEngine.Runtime` 或新的共享程序集。
