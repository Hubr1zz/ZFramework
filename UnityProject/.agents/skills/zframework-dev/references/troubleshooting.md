...
> **适用场景**：编译报错/Player 构建失败/资源加载失败/UI 不显示/事件无响应/性能问题/Luban 配置问题排查 | **关联文档**：[resource-api.md](resource-api.md)、[event-system.md](event-system.md)、[assembly-content-workflow.md](assembly-content-workflow.md)

> 解决新问题后记录到 `.agents/skills/zframework-dev/references/problems/problem_YYYY-MM-DD.md`
...
| 场景 | 常见问题 |
|------|---------|
| 编译/构建 | asmdef 依赖、Editor 正常真机报错、IL2CPP 裁剪、iOS 链接失败、生命周期签名错误 |
...
确认依赖方向是 `GameLogic → GameProto → ZFramework.Runtime`。自定义 asmdef 不能引用 `Assembly-CSharp` 中的类型；需要共享的接口应下沉到 `ZFramework.Runtime` 或新的共享程序集。

#### Editor 正常，真机报错

Editor 通常走 Mono，目标平台可能走 IL2CPP。检查条件编译、代码裁剪和平台 API 差异，开启 `Development Build` + `Script Debugging` 获取完整堆栈。
...
- 程序集、Player 发布和内容包见 [assembly-content-workflow.md](assembly-content-workflow.md)
