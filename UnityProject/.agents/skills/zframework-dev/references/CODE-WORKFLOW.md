...
| L2 调用 | 调用已知 API、单一模块局部修改 | 读取 `zframework-dev` 命中主题 |
...
| 场景 | 必读来源 |
| --- | --- |
| UI 开发 | `zframework-dev/references/ui-lifecycle.md`；复杂 Widget 再读 `ui-patterns.md` |
...
`.agents/skills/zframework-dev/references/problems/problem_YYYY-MM-DD.md`，包含问题现象、来源位置、代码证据和建议修正；具体项目事实同步到相应 reference 或 `project-tooling`。
