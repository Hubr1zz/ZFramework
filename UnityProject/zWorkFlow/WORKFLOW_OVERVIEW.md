...
- **增量维护**：查看需要整理或重构的项目问题。
- **设计文档设置**：选择任意候选目录后，工作台只在找到 `.design-workflow/implementation-ledger.json` 时建立文档包桥接。桥接后会以简化目录树显示每份设计 Markdown 的实现进度、实现后是否修改和修改摘要；绕过文档工作流的指纹变化显示为“手动修改”。右上角状态灯只表示桥接是否有效，不表示是否存在待处理变更。
