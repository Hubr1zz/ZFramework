...
- **增量维护**：查看需要整理或重构的项目问题。
- **设计文档树**：页面顶部只维护一个或多个“设计文档路径”，不再另设“设计文档目录”。至少一个路径有效时，项目在 `openspec/implementation-ledger.json` 创建或读取账本并建立桥接。每个来源显示为独立顶层节点，子目录可逐层折叠。“检查”会重新扫描所有 Markdown、发现新增/删除文件并重算哈希；新增文档默认 0%，且检查不会自动调用设计导入。绕过项目账本的指纹变化显示为“手动修改”。
...
正式 Spec 位于 `openspec/specs/`，描述团队当前认可的功能、规则或架构契约。它不是代码改动日志，也不是某个 Change 的私有副本；只有显式执行 `sync specs <change-id>`，经过合并检查后，Change 的 Delta 才会进入正式 Spec。
