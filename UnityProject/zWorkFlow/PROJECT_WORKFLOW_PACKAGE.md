...
安装 bridge 后，项目 Workbench 只在候选路径中定位到设计包 `.design-workflow/implementation-ledger.json` 时保存本机绑定。它按设计 Markdown 重建简化结构，显示实现进度、实现后变更与摘要；没有摘要的外部指纹变化显示为“手动修改”。右上角灯只表示桥接有效，项目端不写回设计包或自动触发设计导入。
