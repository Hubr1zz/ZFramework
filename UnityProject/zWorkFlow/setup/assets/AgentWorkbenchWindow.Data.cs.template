...
namespace AgentWorkflow.Editor
{
    public sealed partial class AgentWorkbenchWindow : EditorWindow
    {
        ...
        private void ReloadDocumentImplementationChanges()
        {
            ...
            if (!_designDocumentSources.Any(source =>
                    source != null && !string.IsNullOrWhiteSpace(source.path) && Directory.Exists(source.path)))
            {
                _designDocumentTreeItems.Clear();
                _expandedDesignDocumentNodes.Clear();
                _documentChangeStatus = "sync.notConfigured";
                return;
            }
        ...
        {
            var previousDirectoryKeys = new HashSet<string>(
                _designDocumentTreeItems
                    .Where(item => item.isDirectory && !string.IsNullOrWhiteSpace(item.treeKey))
                    .Select(item => item.treeKey),
            ...
            try
            {
                var validSources = _designDocumentSources
                    ...
                    .GroupBy(source => Path.GetFullPath(source.path), StringComparer.OrdinalIgnoreCase)
                    .Select(group => group.First())
                    .ToList();
                var documentFiles = new List<DesignDocumentFileCandidate>();
                foreach (var source in validSources)
                {
                    ...
                    foreach (var file in EnumerateDesignMarkdownFiles(sourceRoot))
                    {
                        ...
                        var relativePath = DocumentRelativePath(absolutePath, sourceRoot);
                        if (string.Equals(relativePath, "AGENTS.md", StringComparison.OrdinalIgnoreCase) ||
                            string.Equals(relativePath, "SETUP.md", StringComparison.OrdinalIgnoreCase))
                            continue;
                ...
                var relativePathCounts = documentFiles
                    .GroupBy(file => file.relativePath, StringComparer.OrdinalIgnoreCase)
                    .ToDictionary(group => group.Key, group => group.Count(), StringComparer.OrdinalIgnoreCase);
                var visibleFiles = documentFiles
                    .OrderBy(file => validSources.IndexOf(file.source))
                    .ThenBy(file => file.relativePath, StringComparer.OrdinalIgnoreCase)
                    .Take(500)
                    .ToList();
                var treeKeys = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

                foreach (var source in validSources)
                {
                    var sourceRoot = Path.GetFullPath(source.path);
                    var sourceKey = $"source:{source.id}";
                    treeKeys.Add(sourceKey);
                    if (!previousDirectoryKeys.Contains(sourceKey))
                        _expandedDesignDocumentNodes.Add(sourceKey);
                    _designDocumentTreeItems.Add(new DesignDocumentTreeItem
                    {
                        ...
                        treeKey = sourceKey,
                        sourceId = source.id,
                        displayName = Or(Path.GetFileName(sourceRoot.TrimEnd(
                            Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)), source.id),
                        absolutePath = sourceRoot
                    });

                    var emittedDirectories = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                    foreach (var file in visibleFiles.Where(file => string.Equals(
                                 file.source.id, source.id, StringComparison.OrdinalIgnoreCase)))
                    {
                        var segments = file.relativePath.Split('/');
                        var directoryPath = string.Empty;
                        var parentKey = sourceKey;
                        for (var index = 0; index < segments.Length - 1; index++)
                        {
                            directoryPath = string.IsNullOrEmpty(directoryPath)
                                ? segments[index]
                                : directoryPath + "/" + segments[index];
                            var directoryKey = $"{sourceKey}:{directoryPath}";
                            treeKeys.Add(directoryKey);
                            if (emittedDirectories.Add(directoryPath))
                            {
                                _designDocumentTreeItems.Add(new DesignDocumentTreeItem
                                {
                                    isDirectory = true,
                                    depth = index + 1,
                                    treeKey = directoryKey,
                                    parentKey = parentKey,
                                    sourceId = source.id,
                                    displayName = segments[index],
                                    relativePath = directoryPath,
                                    absolutePath = Path.Combine(sourceRoot,
                ...
                _expandedDesignDocumentNodes.RemoveWhere(key => !treeKeys.Contains(key));
                if (documentFiles.Count > 500)
                    _documentStructureError = AgentWorkbenchText.Format("sync.documentLimit", 500, documentFiles.Count);
            }
            catch (Exception exception)
            {
        ...
        private static string DocumentRelativePath(string path, string documentRoot)
        {
            return Path.GetRelativePath(documentRoot, path).Replace('\\', '/');
        }
        ...
        private static string ResolveDefaultMaintainer(string projectRoot)
        {
            ...
            var maintainersPath = Path.Combine(projectRoot, ".agent-memory", "zworkflow", "team", "MAINTAINERS.md");
