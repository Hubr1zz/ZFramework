...
在其他项目执行 setup 时，先读取 `setup/adapters/registry.json`，只读分析已有 Agent 工作流并生成多工具能力映射；随后只对已完成共享内容保全和验证的工具专属正文执行薄化，冲突项保持原样。项目耦合的系统资料经代码核验后可增量生成 `system` 正式 Spec；真 Architecture 与 Plugin 进入 `project-tooling`，并按稳定模块边界拆分。
