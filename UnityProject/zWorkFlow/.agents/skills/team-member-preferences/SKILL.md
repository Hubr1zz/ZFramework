...
1. `.agent-memory/zworkflow/team/MAINTAINERS.md`：身份 / 账号 / 环境用户名 → 昵称与规则文件。
2. 当前成员对应的 `.agent-memory/zworkflow/team/members/<nickname>.md`。

不要读取其他成员文件；不要把所有成员规范汇总进入口文档或 skill。

## 任务开始

1. 从 `.agent-memory/zworkflow/team/MAINTAINERS.md` 解析当前用户昵称。
2. 若存在 `.agent-memory/zworkflow/team/members/<nickname>.md`，读取并应用其中规则。
...
- 先确定昵称。
- 将规则追加到 `.agent-memory/zworkflow/team/members/<nickname>.md`。
- 规则保持短句，写清触发场景。
