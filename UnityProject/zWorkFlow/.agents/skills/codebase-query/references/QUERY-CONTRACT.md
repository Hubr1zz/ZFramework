...
- 派生索引：`.agent-memory/zworkflow/local/code-query-index.json`。
- 明确消费者：本 skill、人工 CLI 查询及支持执行 PowerShell 的 Agent。
- 生命周期：源码签名变化时自动更新；指纹包含相对路径、修改时间和大小。未变化文件复用本地结构事实，变化文件只读取和提取一次，随后对紧凑事实重新绑定；文件重命名或移动会移除旧路径并加入新路径。缓存可随时删除。
- 路径格式：索引和 JSON 输出统一保存 `/` 分隔的项目相对路径，访问文件时转换为当前操作系统的目录分隔符；源码、索引和控制台输入输出显式使用 UTF-8。

索引只保存文件路径、命名空间、完整类型名、继承关系、方法签名、调用标识符、
...
每次输出包含 `engine=codebase-query-regex-binding-v5` 与 `schemaVersion=5`，供 Agent 和人工核验查询来源。构建进度输出到 stderr 和本地进度快照，stdout 始终只包含最终 JSON。索引通过同目录临时文件原子替换，构建中断不会留下半份正式缓存。
...
索引 v5 使用单文件一次提取、文件级增量复用、全量覆盖校验和轻量 C# 类型绑定，但仍不是 Roslyn 语义模型。
