...
`tooling-catalog.json` 是 Git 同步的项目事实与策略权威源。顶层包含 `schemaVersion`、`updatedAt` 与 `entries`。每个条目包含稳定 `id`、`displayName`、`displayNameEn`、`kind`、`description`、`descriptionEn`、`version`、`source`、`usagePolicy`、可选的 `usageNotes`/`usageNotesEn`、`decisionBasis`、`locked`、`capabilities`、`capabilitiesEn`、`constraints`、`constraintsEn`、`evidence` 和 `dependencies`。
