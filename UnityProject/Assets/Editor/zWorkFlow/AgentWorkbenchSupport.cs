...
namespace AgentWorkflow.Editor
{
    ...
    internal static class AgentWorkbenchText
    {
        ...
        private static readonly Dictionary<string, (string Zh, string En)> Values = new()
        {
            ...
            ["sync.bridgeHint"] = ("项目侧根据所有设计文档路径维护结构树和实现后变更账本；刷新不会自动调用设计导入或创建 Proposal。", "The project builds the tree and maintains the post-implementation change ledger from all document sources. Refresh never invokes Design Import or creates proposals automatically."),
            ...
            ["sync.detectedByFingerprint"] = ("当前文档指纹与实现基线不同；项目账本尚未补充变更摘要。", "The current document fingerprint differs from the implementation baseline; the project ledger has not added a change summary yet."),
