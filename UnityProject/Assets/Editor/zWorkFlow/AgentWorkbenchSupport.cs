...
namespace AgentWorkflow.Editor
{
    ...
    internal static class AgentWorkbenchText
    {
        ...
        private static readonly Dictionary<string, (string Zh, string En)> Values = new()
        {
            ...
            ["toolbar.designDocumentTree"] = ("设计文档树", "Design Document Tree"),
            ["toolbar.openRoot"] = ("在资源管理器中打开", "Open in File Explorer"),
            ["toolbar.projectRoot"] = ("项目根目录", "Project Root"),
            ["toolbar.designDocumentRoot"] = ("设计文档路径", "Design Document Paths"),
            ["toolbar.commandList"] = ("指令列表", "Command List"),
            ...
            ["codeIndex.idle"] = ("尚未执行构建。", "No build has been run yet."),
            ["codeIndex.running"] = ("正在构建：{0}", "Building: {0}"),
            ["codeIndex.succeeded"] = ("全量索引构建成功。", "Full index build succeeded."),
            ["codeIndex.failed"] = ("索引构建失败。", "Index build failed."),
            ...
            ["codeIndex.elapsed"] = ("耗时", "Elapsed"),
            ["codeIndex.missing"] = ("缺失", "Missing"),
            ["codeIndex.unexpected"] = ("异常", "Unexpected"),
            ["codeIndex.diskMismatch"] = ("磁盘当前有 {0} 个 C# 文件，索引记录 {1} 个；请重新构建。", "Disk currently has {0} C# files while the index records {1}; rebuild is required."),
            ["codeIndex.fullScope"] = ("范围：Assets/**/*.cs（包含 Plugins、ThirdParty、External 与 Standard Assets）", "Scope: Assets/**/*.cs (including Plugins, ThirdParty, External, and Standard Assets)"),
            ["engineering.title"] = ("工程能力目录", "Engineering Capability Catalog"),
            ["engineering.summary"] = ("类型描述能力的归属与复用边界；可选分层描述能力横跨的实现层，两者互不替代。", "Kind describes ownership and reuse boundaries; optional layers describe where a capability is implemented. These are independent dimensions."),
            ...
            ["sync.package"] = ("设计文档路径", "Design Document Paths"),
            ...
            ["sync.bridgeHint"] = ("项目侧根据设计文档路径构建结构树，并仅消费校验有效的实现路由摘要；刷新不会自动调用设计导入或创建 Proposal。", "The project builds the document tree and consumes only a validated implementation routing summary. Refresh never invokes Design Import or creates proposals automatically."),
            ["sync.routingIndexCurrent"] = ("实现路由索引有效", "Implementation routing index is current"),
            ["sync.routingIndexMissingOrStale"] = ("实现路由索引缺失或已过期；请运行进度刷新", "Implementation routing index is missing or stale; refresh progress"),
            ["sync.routingIndexError"] = ("实现路由索引错误：{0}", "Implementation routing index error: {0}"),
            ["sync.notConfigured"] = ("未配置", "Not configured"),
            ["sync.packageNotSelected"] = ("尚未添加有效的设计文档路径", "No valid design document path has been added"),
            ["sync.packageKeyNotFound"] = ("所选路径中未找到可桥接的 Markdown 文档，未记录该路径。", "No bridgeable Markdown documents were found in the selected path. The path was not saved."),
            ["sync.packageFound"] = ("已连接设计文档目录：{0}", "Design document directory connected: {0}"),
            ["sync.noMarkdownDocuments"] = ("所选路径中未找到 Markdown 文档，未记录该路径。", "No Markdown documents were found in the selected path. The path was not saved."),
            ["sync.pathMissing"] = ("路径不存在", "Path does not exist"),
            ["sync.ledgerMissing"] = ("项目实现后变更账本尚未创建", "The project post-implementation change ledger has not been created"),
            ...
            ["sync.statusVerified"] = ("已验证", "Verified"),
            ["sync.statusStale"] = ("需复核", "Stale"),
            ["sync.statusBlocked"] = ("阻塞", "Blocked"),
            ["sync.statusInProgress"] = ("实现中", "In Progress"),
            ["sync.statusNotImplemented"] = ("未实现", "Not Implemented"),
            ["sync.verificationVerified"] = ("验证通过", "Verified"),
            ["sync.verificationImplemented"] = ("待验证", "Needs Verification"),
            ["sync.verificationFailed"] = ("验证失败", "Verification Failed"),
            ["sync.verificationUnverified"] = ("未验证", "Unverified"),
            ["sync.connectPackageToViewStructure"] = ("添加至少一个有效的设计文档路径后显示文档结构。", "Add at least one valid design-document path to view its structure."),
            ...
            ["sync.open"] = ("打开", "Open"),
            ["sync.configurePackageFirst"] = ("请添加至少一个包含 Markdown 的设计文档路径。", "Add at least one design-document path containing Markdown."),
            ...
            ["sync.choosePackage"] = ("选择设计文档目录", "Select Design Document Directory"),
            ["sync.refreshDocumentStatus"] = ("刷新文档状态", "Refresh Document Status"),
            ...
            ["cmd.checkDocumentFreshness"] = ("检查文档及时性", "Check Document Freshness"),
            ["cmd.checkDocumentFreshnessDesc"] = ("重新检查已实现设计文档是否在实现后发生修改；只刷新状态和摘要，不会自动设计导入或创建 Proposal。", "Recheck whether implemented design documents changed afterward. This refreshes status and summaries only; it never starts a design import or creates a proposal."),
            ...
            ["cmd.translateSpecs"] = ("翻译现有Spec：中文|英文 [范围]", "Translate Existing Specs: Chinese|English [scope]"),
            ["cmd.translateSpecsDesc"] = ("为指定范围同步非权威语言显示副本；权威 Spec 原文保持不变。", "Synchronize non-authoritative display copies for a scope without changing canonical Specs."),
            ["cmd.syncSpecTranslations"] = ("同步Spec翻译：中文|英文 [范围]", "Sync Spec Translations: Chinese|English [scope]"),
            ["cmd.syncSpecTranslationsDesc"] = ("在权威 Spec 更新后刷新指定语言的翻译块和 hash 索引。", "Refresh translation blocks and hash indexes for a language after canonical Specs change."),
