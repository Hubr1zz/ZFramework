...
namespace HuntingInDarkness.ActionFlow.Combat
{
    ...
    public sealed class RestoreCharacterCardAction : CompositeGameAction, ISourceAction, ITargetAction
    {
        ...
        public RestoreCharacterCardAction(CharacterActionCardInstance card, ActionCardCostService costService, FlipConditionEvaluator flipEvaluator, ActionEventOutbox eventOutbox, IReactorEntity source, IReactorEntity target)
        {
        ...
        protected override GameAction GetNextChild(CompositeExecutionContext context)
        {
            ...
            if (!preparationCompleted || finalizationScheduled) return null;
            finalizationScheduled = true;
            return new FinalizeCardRestoreAction(this);
        }
        ...
        internal ActionOutcome Commit()
        {
            if (!flipEvaluator.CanManuallyRestore(card)) return ActionOutcome.Failure("行动卡恢复条件已发生变化");
            if (transaction != null && !transaction.TryCommit(card.OwnerCharacterId, costService)) return ActionOutcome.Cancelled("恢复费用已发生变化");
    ...
    public sealed class FinalizeCardRestoreAction : CommandAction, ISourceAction, ITargetAction
    {
        ...
        protected override UniTask<ActionOutcome> ExecuteAsync(ActionExecutionContext context, CancellationToken cancellationToken) => UniTask.FromResult(root.Commit());
    }
    ...
    public sealed class BurstCharacterCardAction : CompositeGameAction, ISourceAction, ITargetAction
    {
        ...
        public BurstCharacterCardAction(CharacterActionCardInstance card, IGameContext gameContext, ActionEventOutbox eventOutbox, Action<int, int> applyTimePointReward, IReactorEntity source, IReactorEntity target)
        {
            this.card = card ?? throw new ArgumentNullException(nameof(card));
        ...
        protected override GameAction GetNextChild(CompositeExecutionContext context)
        {
            ...
            finalizationScheduled = true;
            return new FinalizeCardBurstAction(this);
        }
