...
namespace Core
{
    ...
    public class GameManager : MonoBehaviour, IGameContext, ICombatProvider, ICombatInspirationReadModel, IPlayableActionCardCommandSink, ICombatRuntimeDataProvider, ICampaignPhaseTransitionHost, ICampaignPhaseTransitionRequestHost, IPlayableHuntRetreatInput, ISettlementDepartureRequestPort
    {
        ...
        private System.IDisposable inventionActionInstallation;
        ...
        public async UniTask<CampaignStartupResult> ContinueCampaignAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (!SettlementManager.TryPrepareCandidate(snapshot.Settlement, out SettlementManager candidateSettlementManager, out string projectionReason)) return CampaignStartupResult.Failed(CampaignStartupState.AwaitingChoice, projectionReason);
                CampaignSnapshot candidateSnapshot = ActiveHuntSnapshotAdapter.CaptureSettlement(candidateSettlementManager.Data);
                if (!SaveLoadSystem.TryCreatePayload(candidateSnapshot, out string candidatePayload, out projectionReason)) return CampaignStartupResult.Failed(CampaignStartupState.AwaitingChoice, projectionReason);
                if (!TryStartCampaignRuntime(GamePhase.Settlement, false, out string startReason, candidateSettlementManager, false)) return CampaignStartupResult.Failed(CampaignStartupState.AwaitingChoice, startReason);
                bool restored = await FinalizePreparedSettlementAsync(candidateSettlementManager.Data, candidatePayload);
                if (!restored)
                {
                    ResetFailedCampaignStartupRuntime();
        ...
        private bool TryStartCampaignRuntime(GamePhase startPhase, bool queueSettlementEvents, out string reason, SettlementManager preparedSettlement = null, bool activateOnSuccess = true)
        {
            reason = string.Empty;
            if (campaignStarted)
            {
            ...
            try
            {
                ...
                {
                    if (!TryCommitSettlementCandidate(preparedSettlement, null, out reason)) return false;
                }
                else
        ...
        {
            inventionActionInstallation ??= actionEnvironmentInstallers.Register(new InventionActionEffectInstaller(() => _settlementManager?.Data, () => _settlementManager?.Inventions?.AllInventions));
        ...
        private void ResetFailedCampaignStartupRuntime()
        {
            ...
            campaignActionSession?.Dispose();
            campaignActionSession = null;
            inventionActionInstallation?.Dispose();
            inventionActionInstallation = null;
            _settlementManager = null;
            _huntMgr = null;
        ...
        private void StartSettlementActionSession()
        {
            ...
            settlementActionSession = new PlayableSettlementActionSession(_settlementManager.Data, new PlayableWeaponTrainingContentAdapter(PlayableWeaponMasteryRuntime.Catalog), _settlementManager.Events, playableEventInput, new PlayableSettlementCareContentAdapter(settlementContentCatalog), new PlayableSettlementEquipmentContentAdapter(PlayableSettlementContentRuntime.Items), tabletopInteractionRouter, _settlementManager.Workshop, _settlementManager.Inventions, workshopContentCatalog, PlayableSymptomRuntime.Catalog, actionEnvironmentInstallers, _settlementManager.Timeline.ResolveEvent, _settlementManager.Timeline);
        }
        ...
        private bool TryEnterHuntPhase(IReadOnlyList<HunterInstance> committedRoster, bool allowDevelopmentFallback, CampaignHuntEntryContext huntContext, SettlementInstance expectedSettlement, out string reason)
        {
            ...
            try
            {
                ...
                {
                    if (!_huntMgr.TryBindContent(huntContext.RoutePlan, out reason))
                    {
                        _huntMgr = null;
                        return false;
                    }
                }
                else if (!PlayableHuntDestinationRuntime.TryApplyTo(_huntMgr, out reason))
                {
                    _huntMgr = null;
                    return false;
                }
        ...
        private HuntManager CreateHuntManager(SettlementManager settlementManager)
        {
            ...
            manager.OnBossEncounterTriggered = () =>
            {
                ...
                var request = new CampaignEncounterRequest(huntActionSession.SessionId, PlayableEncounterRuntime.DefaultEncounterId, CampaignEncounterSourceKind.HuntBossTile, GamePhase.Hunt, _huntMgr.SquadPosition, string.Empty, _huntMgr.BoundRoute?.DestinationId ?? string.Empty);
        ...
        private bool TryRestoreActiveHunt(CampaignSnapshot campaign, out string reason)
        {
            ...
            if (TryStartHuntPresentationAndSession(restoredOccurrences, out reason))
            {
                ...
                {
                    Debug.LogWarning($"[GameManager] 退役旧狩猎 ActionSession 时发生异常，活动狩猎恢复结果仍然有效：{exception.Message}");
                }
                return true;
            }
        ...
        private void RestoreActiveHuntRuntime(SettlementManager settlementManager, HuntManager huntManager, PlayableSettlementActionSession settlementSession, PlayableHuntActionSession huntSession, SettlementEventRestoreProjection restoreProjection, string expeditionId, PlayableHuntDestinationRuntime.RuntimeState destinationState)
        {
        ...
        public bool IsCampaignActionSessionActive => campaignStarted && campaignActionSession?.IsActive == true;
        ...
        public UniTask<CampaignPhaseTransitionResult> TransitionToPhaseAsync(CampaignPhaseTransitionRequest request)
        {
            if (!campaignStarted)
                return UniTask.FromResult(CampaignPhaseTransitionResult.Failed(CurrentGamePhase, "战役入口尚未完成。"));
            if (campaignActionSession?.IsActive == true)
                return campaignActionSession.TransitionAsync(request, this.GetCancellationTokenOnDestroy());
            GamePhase previousPhase = CurrentGamePhase;
            string reason = request.IsValid ? string.Empty : "战役阶段切换请求无效。";
            if (request.IsValid && ((ICampaignPhaseTransitionRequestHost)this).TryApplyPhaseTransition(request, out reason))
                return UniTask.FromResult(new CampaignPhaseTransitionResult(true, previousPhase != CurrentGamePhase, previousPhase, CurrentGamePhase, string.Empty));
            return UniTask.FromResult(CampaignPhaseTransitionResult.Failed(CurrentGamePhase, string.IsNullOrWhiteSpace(reason) ? "战役流程尚未准备完成。" : reason));
        }
