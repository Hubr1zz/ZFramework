...
主动恢复与灵感爆发也已成为 Combat Root。恢复在准备阶段选择并锁定灵感费用，提交时重新校验卡面与恢复条件；爆发先准备/展开奖励效果，再以不可阻止的最终节点发布翻面和弃置检查点，最后应用时点返还。并发点击由同一 Runner 串行重验，只有一个请求能够提交。自动回合恢复及“其他卡翻面/恢复/弃置”联动仍由 `FlipConditionEvaluator` 的 EventBus 监听直接修改状态，需在阶段 4 前迁成事件来源 Root 的后代 Action。
