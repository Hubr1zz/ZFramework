...
- 年度推进已从“每年累计多次出猎”收敛为“每个稳定 HuntRecord 归来一次推进一年”：Campaign 接受回营后先持久化 PendingHuntReturn，Settlement Runner 再幂等提交 HuntHistory、年份与年度 Timeline；失败或读档恢复期间所有出猎入口保持门禁。旧配额字段仅作为一次性存档迁移输入。
...
21. Hunt 回营的跨 Runner 交接已有持久化 PendingHuntReturn 和稳定 RecordId，但资源转移、成长结算与检查点赋值仍由 `GameManager.CommitPreparedHuntExit` 在同一同步调用中串联，磁盘保存随后异步进行。当前原子存档与版本门禁能保证磁盘只落在旧 Hunt 快照或完整待处理快照；完整 `CampaignFlowCoordinator` 落地时仍应把该交接改为显式计划与结果对象，避免新增跨域副作用插入检查点之前。
