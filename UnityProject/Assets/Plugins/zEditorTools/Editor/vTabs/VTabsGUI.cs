        ...
        public EditorWindow AddTab(TabInfo tabInfo, bool atOriginalTabIndex = false)
        {
            ...
            void setupBrowser()
            {
                ...
                void lockToFolder_twoColumns()
                {
                    ...
                    var iid = AssetDatabase.LoadAssetAtPath<Object>(AssetDatabase.GUIDToAssetPath(tabInfo.folderGuid)).GetLegacyInstanceId();

#if UNITY_6000_3_OR_NEWER
                    window.GetFieldValue("m_ListAreaState").SetFieldValue("m_SelectedInstanceIDs", new List<EntityId> { iid.ToEntityId() });
#else
                ...
                void lockToFolder_oneColumn()
                {
                    ...
                    var folderIid = AssetDatabase.LoadAssetAtPath<Object>(folderPath).GetLegacyInstanceId();

#if UNITY_6000_3_OR_NEWER
                    data.SetMemberValue("m_rootInstanceID", folderIid.ToEntityId());
#else
