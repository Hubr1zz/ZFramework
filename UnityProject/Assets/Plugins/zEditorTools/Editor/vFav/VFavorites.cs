...
namespace VFavorites
{
    ...
    {
        public static bool IsShortcutPressed => shortcutPressed;
        ...
        static void WrappedOnGUI(object _)
        {
            ...
            void doOriginalGUIFirst_()
            {
                ...
                if (VFavoritesMenu.embeddedPanelUsesWindowUI)
                {
                    VFavoritesWindow.DrawEmbedded(embeddedRect_browserSpace, currentOpacity);
                    return;
                }
            ...
            void doVFavoritesGUIFirst_()
            {
                if (VFavoritesMenu.embeddedPanelUsesWindowUI)
                {
                    ...
                    if (embeddedRect_browserSpace.IsHovered())
        ...
        static void SelectItem(Item item)
        {
            void openFolder(EditorWindow browser, string path)
            {
                ...
                if (browser.GetFieldValue<int>("m_ViewMode") == 1)
                    browser.InvokeMethod("SetFolderSelection", new[] { folderAsset.GetLegacyInstanceId() }, false);
                else
                {
                    Selection.activeObject = folderAsset;

                    browser.GetMemberValue("m_AssetTree")?.GetPropertyValue("data")?.InvokeMethod("SetExpanded", folderAsset.GetLegacyInstanceId(), true);
        ...
        static Rect embeddedRect_browserSpace => wrappedBrowser.position.SetPos(0, 0);
        ...
        static void UpdateGUIWrapping() // called from EditorApplicaton.update 
        {
            ...
            void unwrap()
            {
                if (!wrappedBrowser)
                {
                    if (!shortcutPressed)
                    {
                        VFavoritesWindow.ReleaseEmbeddedRenderer();
                        VFavoritesWindow.CleanupInvalidWindows();
                    }
        ...
        static void UpdateLocking() // called from EditorApplicaton.update 
        {
            ...
            void lock_()
            {
                ...
                EditorPrefsCached.SetInt("vFavorites-lockedBrowserDockAreaInstanceId", lockedBrowser.GetMemberValue<Object>("m_Parent").GetLegacyInstanceId());
        ...
        static EditorWindow lockedBrowser
        {
            get
            {
                ...
                var window = LegacyInstanceIdToObject(lockedBrowserInstanceId) as EditorWindow;
            ...
            set
            {
                ...
                {
                    EditorPrefsCached.SetInt("vFavorites-lockedBrowserInstanceId", value.GetLegacyInstanceId());
        ...
        static void Init()
        {
            ...
            void subscribe()
            {
                ...
                EditorApplication.delayCall -= VFavoritesWindow.CleanupInvalidWindows;
                EditorApplication.delayCall += VFavoritesWindow.CleanupInvalidWindows;
