...
namespace ZFramework.RTS.Editor
{
    ...
    internal sealed class RtsCompileUnitSettings
    {
        ...
        [SerializeField] internal string outputDirectory = "Library/ZFrameworkRTS/Compiled";
    }

    [FilePath("ProjectSettings/ZFrameworkRtsSettings.asset", FilePathAttribute.Location.ProjectFolder)]
    internal sealed class RtsProjectSettings : ScriptableSingleton<RtsProjectSettings>
    {
        ...
        internal IEnumerable<string> ResolveSourceRoots()
        {
            ...
            foreach (string path in ActiveUnit.sourceRoots)
            {
                ...
                if (normalized.Equals("Packages/com.zframework.rts/UserScripts~", StringComparison.OrdinalIgnoreCase)) continue;
        ...
        {
            yield return Path.Combine(ProjectRoot, "Library", "ScriptAssemblies", "ZFramework.RTS.Contracts.dll");
        ...
        internal string ResolveOutputDirectory()
        {
            ...
            if (configured.Replace('\\', '/').StartsWith("Packages/com.zframework.rts/Temp~/", StringComparison.OrdinalIgnoreCase))
                configured = "Library/ZFrameworkRTS/Compiled";
    ...
    internal static class RtsSettingsProvider
    {
        ...
        {
            return new SettingsProvider("Project/ZFramework RTS", SettingsScope.Project)
            {
                label = "ZFramework RTS",
                guiHandler = _ => DrawSettings(),
                keywords = new HashSet<string>(new[] { "ZFramework", "RTS", "Roslyn", "Workspace" })
            };
        }
