...
$contractAssembly = Join-Path $projectRoot 'Library/ScriptAssemblies/TEngine.RTS.Contracts.dll'
...
dotnet run --project (Join-Path $PSScriptRoot 'Compiler/TEngine.RTS.Compiler.csproj') --configuration $Configuration -- `
    --source $sourceRoot `
    --output (Join-Path $projectRoot "Library/TEngineRTS/Compiled/Sessions/$Session") `
