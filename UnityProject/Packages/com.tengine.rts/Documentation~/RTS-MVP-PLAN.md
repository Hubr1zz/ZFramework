# TEngine RTS 维护计划与测试指南

## 1. 定位

RTS 是 Editor 优先的逻辑实验层，不是 Player 热更新或安全沙箱。Unity/TEngine 负责场景、资源、物理和主循环；RTS 负责普通 C# 玩法实例的创建、Tick、状态迁移和快速替换。
...
- Unity 无关的 `TEngine.RTS.Contracts`。
...
- 重新加载当前 TEngine 主场景，同时保留 Provider、ModuleSystem 和启动/资源流程。
...
- Unity Test Framework Play Mode 集成测试：完整 TEngine 启动、RTS 场景加载、脚本 Tick 和三次场景重启。
...
`RestartCurrentScene` 使用 TEngine `ISceneModule.LoadSceneAsync` 以 `Single` 模式重新加载 `CurrentMainSceneName`：
...
2. TEngine 的持久化 GameEntry、RootModule、ModuleSystem、资源包和当前 RTS Provider 保留。
...
1. 回到 Edit Mode。
2. 打开 `TEngine > RTS > Control Center`，在“手动工具”选择 `RtsTest` 并启动。
3. 工具会补齐 YooAsset 所需目录、生成 `Assets/AssetRaw/Scenes/RTSTest.unity`、编译当前 Session 脚本并从 `Assets/Scenes/main.unity` 启动。
4. 完整 TEngine Procedure 到达 `ProcedureStartGame` 后，工具通过 `ISceneModule` 加载 `RTSTest`，并装载最新 RTS DLL。
...
```powershell
dotnet run --project UnityProject/Packages/com.tengine.rts/Tools~/KernelSmokeTests/TEngine.RTS.KernelSmokeTests.csproj --configuration Release
```

打开 Unity 或用 BatchMode 编译一次，使下面文件存在：

```text
UnityProject/Library/ScriptAssemblies/TEngine.RTS.Contracts.dll
```

然后在 `UnityProject` 目录执行：

```powershell
pwsh -File Packages/com.tengine.rts/Tools~/compile-sample.ps1 -Configuration Release -Session '<SessionId>'
dotnet build Packages/com.tengine.rts/Tools~/ProductionCompileSmoke/TEngine.RTS.ProductionCompileSmoke.csproj `
...
1. 打开一个通过 TEngine 正常启动后进入的玩法场景，不要停留在 Launcher/build index 0。
...
7. 选择 `Library/TEngineRTS/Compiled/Sessions/<Session>/` 中最新 DLL。
...
### 6.1 与 TEngine 启动模式的关系

RTS 适合进入 TEngine Editor 扩展的启动选项，但应作为独立“启动目标”，不扩展 YooAsset `EPlayMode`：
...
两组选项正交。当前 `Start TEngine Test Flow` 是启动目标的最小实现；未来可在现有 TEngine Inspector/Toolbar 中复用该命令，不修改资源模式枚举或运行时资源契约。
...
- Player 不包含 `TEngine.RTS.Contracts`、`TEngine.RTS.Runtime` 或任何 Editor/Roslyn 编译程序集。
- 正式场景和 Prefab 不保留 `ScriptAnchor`。
- 正式源码不实现 `IScript`，不保留 `ScriptIdAttribute`、动态/静态 Provider、状态热迁移和 `RtsStaticRegistry`。
- 正式玩法逻辑转换为普通项目 C#，由传统 MonoBehaviour、TEngine Module/Procedure 或项目自己的生命周期入口驱动。
- 构建前扫描程序集、场景、Prefab 和生成目录；发现 RTS 类型或引用时直接阻止正式构建。

当前实现：`Contracts` 与 `Runtime` asmdef 均限制为 Editor；外部 Roslyn 编译定义 `TENGINE_RTS`；正式导出只让 Unity 的传统入口分支生效。构建前 `RtsZeroBuildGuard` 检查 Player 程序集、旧注册表目录、活动 Generated 源码，以及启用 Build Scene 对 RTS Runtime/Contracts 的依赖。未加入 Build 的 RTSTest 可以继续作为开发资产保留。
...
1. 调用 `TEngine.RTS.Editor.RtsTestFlow.PrepareTestAssetsForBatch`，生成测试场景及所有权标记。
2. 运行 Kernel、当前 Session 的 Roslyn 编译和接入项目自有的集成测试。
3. 调用 `TEngine.RTS.Editor.RtsSourcePromotion.ExportZeroRtsForBatch`，按 session/version 生成增量正式源码，不修改场景。
4. 调用 `TEngine.RTS.Editor.RtsZeroBuildGuard.ValidateForBatch`。
5. 执行真实 Player/IL2CPP 构建，并在 CI 检查最终产物中不存在 `TEngine.RTS` 程序集或类型名。
...
- [x] 通过 ResourceModuleDriver Inspector 扩展接入 TEngine 启动目标，同时保持启动目标与 YooAsset 资源模式正交。
- [x] 正式化增加 Dry Run、文件变更/删除清单、确认和 `Library/TEngineRTS/PromotionBackups` 可恢复备份；保留未变化脚本的 `.meta`/GUID。
...
- `TEngine > RTS > Control Center`：统一启动、编译、重启和正式化工作台。
- `Project Settings > TEngine RTS`：配置编译单元、源码目录、引用程序集、输出目录和主场景。
