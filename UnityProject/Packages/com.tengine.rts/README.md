# TEngine Roslyn Runtime Scripts
...
- Data：唯一规则与状态所有者，纯 C#，不引用 Unity、TEngine 或 RTS。
...
唯一菜单入口是 `TEngine > RTS > Control Center`。主页签服务 Agent 工作流，手动恢复、正式化和文档分别位于独立页签。项目级配置在 `Project Settings > TEngine RTS`。
