...
namespace TEngine.RTS.Editor
{
    internal sealed class RtsControlCenter : EditorWindow
    {
        ...
        [MenuItem("TEngine/RTS/Control Center", priority = -100)]
        internal static void Open()
        {
            var window = GetWindow<RtsControlCenter>("TEngine RTS");
        ...
        private void DrawManualTools()
        {
            ...
            DescribeButton("项目设置", "编辑编译单元、源码根、主场景、session 和 Agent 边界。", () => SettingsService.OpenProjectSettings("Project/TEngine RTS"));
        ...
        private static void DrawRuntimeSummary()
        {
            ...
            if (EditorApplication.isPlaying)
            {
                ...
                catch (Exception) { generation = "等待 TEngine 初始化"; }
            }
        ...
        {
            string absolute = Path.Combine(RtsProjectSettings.instance.ProjectRoot, "Packages/com.tengine.rts", relativePath);
    ...
    internal static class RtsResourceInspectorExtension
    {
        ...
        private static void DrawHeader(UnityEditor.Editor editor)
        {
            ...
            {
                EditorGUILayout.LabelField("TEngine RTS", EditorStyles.boldLabel);
