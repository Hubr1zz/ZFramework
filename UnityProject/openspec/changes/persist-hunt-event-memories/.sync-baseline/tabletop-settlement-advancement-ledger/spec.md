...
营地 SHALL 提供年鉴入口卡，显示当前年份、最近一次远征的年度跃迁与记录数；年鉴板 SHALL 把时间线和狩猎历史合并为按年份排序的分页实体条目。

已掌握的发明 SHALL 作为已完成的长期记录进入年鉴；狩猎历史中持久化的物品 ContentId SHALL 在 View 边界解析为玩家可读名称，不得直接显示技术标识。
...
- **WHEN** 稳定狩猎记录已经提交并使营地从完成年度推进到下一年
- **THEN** 年鉴入口卡和已打开的年鉴 SHALL 立即显示“完成年度 → 当前年度”与新的总记录数，不得显示年度出猎配额
