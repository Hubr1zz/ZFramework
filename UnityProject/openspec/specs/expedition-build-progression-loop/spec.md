...
狩猎事件或采集获得的资源 SHALL 先进入本次远征携带物；成功撤退后，Settlement 回营 Action SHALL 将其一次性提交到营地库存、记录远征并推进一个配置季节。只有越过冻结日历的最后季节时才推进一年。制造或装备操作不得在有效回营检查点清除前开始。

#### Scenario: Black salt returns from an expedition

- **WHEN** 猎人在狩猎中获得一份 `black_salt` 并成功撤退
- **THEN** 资源 SHALL 从远征携带物转入营地库存，回营记录 SHALL 被应用且日历游标 SHALL 恰好推进一个配置季节
- **AND** 当前年份 SHALL 仅在该季节推进越过冻结日历末尾时增加一
